...
export default function TicketCategoriesMasterView() {
    const { ticketCategories, isLoading, createTicketCategory, deleteTicketCategory } = useMasters();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [formData, setFormData] = useState({ name: '', description: '', defaultPriority: '' });
    ...
    const handleDelete = async (id: number) => {
        if (confirm('Delete this ticket category?')) await deleteTicketCategory(id);
    };

    const handleCloseModal = () => {
        setIsModalOpen(false);
        setFormData({ name: '', description: '', defaultPriority: '' });
    };
        ...
        <>
            <Card className="border-none shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 overflow-hidden h-[600px] flex flex-col p-0">
                <CardHeader className="p-4 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center border-b border-slate-200 dark:border-slate-700 mb-0">
                    <h3 className="font-bold text-slate-800 dark:text-slate-100">Ticket Categories List ({ticketCategories?.length || 0})</h3>
                            ...
                            <table className="w-full border-collapse border border-slate-200 dark:border-slate-700">
                                ...
                                <tbody className="bg-white dark:bg-gray-900">
                                    {ticketCategories?.length === 0 ? (
                                        <tr><td colSpan={5} className="p-8 text-center text-slate-500">No ticket categories found</td></tr>
                                    ) : (
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Add Ticket Category">
