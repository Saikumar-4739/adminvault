...
export default function MastersPage() {
    ...
    useEffect(() => {
        switch (activeTab) {
            ...
            case 'designations': fetchDesignations(); break;
    ...
    }, [activeTab, fetchDepartments, fetchDesignations, fetchAssetTypes, fetchBrands, fetchVendors, fetchLocations, fetchTicketCategories]);
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="min-h-[500px]">
                ...
                {activeTab === 'designations' && <GenericMasterView title="Designations" data={designations} onDelete={deleteDesignation} onCreate={createDesignation} type="designation" isLoading={mastersLoading} />}
...
function CompaniesMasterView() {
    ...
    return (
        <Card className="border-none shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 overflow-hidden min-h-[400px]">
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                <h3 className="font-bold text-slate-800 dark:text-slate-100">Registered Companies</h3>
            </div>
            {isLoading ? (
                <PageLoader />
            ) : (
        ...
        <>
                                        ...
                                        <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                            ...
                                            <td className="px-4 py-3 text-center border border-slate-200 dark:border-slate-700 text-sm">
                                                <div className="flex justify-center gap-2">
                                                    <button onClick={() => handleDelete(item.id)} className="text-rose-600 hover:text-rose-800 text-xs font-medium">Delete</button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Add ${title.slice(0, -1)}`}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <Input label="Name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder="Enter Name" />

                    {type === 'department' && <Input label="Code" value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} placeholder="e.g. IT" />}
                    {type === 'designation' && <Input label="Level" value={formData.level} onChange={(e) => setFormData({ ...formData, level: e.target.value })} placeholder="e.g. L3" />}
                    {type === 'vendor' && (
                        <>
