...
export class DashboardService {
    ...
    async getDashboardStats(): Promise<DashboardStatsResponseModel> {
        try {
            const [ assetStats, ticketStats, employeeStats, licenseStats ] = await Promise.all([ this.repository.getAssetStats(), this.repository.getTicketStats(), this.repository.getEmployeeStats(), this.repository.getLicenseStats()]);
