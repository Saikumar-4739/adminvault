...
@Module({
    imports: [ConfigModule, forwardRef(() => AdministrationModule), PassportModule.register({ defaultStrategy: 'jwt' }), JwtModule.register({ secret: process.env.JWT_SECRET || SECRET_KEY, signOptions: { expiresIn: '7d' } }), TypeOrmModule.forFeature([AuthUsersEntity, AuthTokensEntity, AccessRequestEntity])],
